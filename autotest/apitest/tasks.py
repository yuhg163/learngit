...
#             request_urls.append(new_url)
        ...
        if method.upper() == 'GET':
            ...
            if "=" in urlParam(param):
                data = None
                print ('caseId:'+str(case_id)+'\nrequest is get:' +new_url+'?'+urlParam(param)+'\nbody is:'+urlParam(param))
                try:
                    results = requests.get(new_url+'?'+urlParam(param),data,headers=headers).text   #str
                    print ('response is get:\n'+results)
                except Exception as e:
                    writeBug(case_id,interface_name,new_url,str(e),res_check,Product_id,apitester)
                    return caseWriteResult1(case_id,str(e),'0')                 
                responses.append(results)
                res = readRes(results,res_check)
                print(res)
            else:               
                print ('caseId:'+str(case_id)+'\nrequest is get:' +new_url+'\nbody is:'+urlParam(param))  
                data = None
                req = urllib.request.Request(url=new_url,data=data,headers=headers,method="GET")
                try:
                    results = urllib.request.urlopen(req).read().decode()   #byte.decode()
                except Exception as e:
                    writeBug(case_id,interface_name,new_url,str(e),res_check,Product_id,apitester)
                    return caseWriteResult1(case_id,str(e),'0') 
                print('response is get:')
            ...
            if 'pass' == res:                
                ...
                writeBug(case_id,interface_name,new_url,results,res_check,Product_id,apitester)
        if  method.upper()=='PUT':
       ...
       #         writeResult(case_id,'pass')
        ...
        if method.upper()=="POST":
            ...
            if "=" in urlParam(param):
                data = None
                print ('caseId:'+str(case_id)+'\nrequest is get:' +new_url+'\nbody is:'+urlParam(param)) 
                req = urllib.request.Request(url=new_url+'?'+urlParam(param),data=data,headers=headers,method="POST")
                try:
                ...
                res = readRes(results,res_check)
                print(res)    
#             elif "{" in urlParam(param):
# #                 data = json.dumps(urlParam(param)).encode()
...
def caseWriteResult1(case_id,response,result):
    ...
    print ('api autotest result is '+result)
    coon = pymysql.connect(user=getDatabases('user'),passwd=getDatabases('passwd'),db=getDatabases('db'),port=getDatabases('port'),host=getDatabases('host'),charset=getDatabases('charset'))
    cursor = coon.cursor()
...
def writeBug(bug_id,interface_name,request,response,res_check,Product_id,tester):
