...
class Webcasestep(models.Model):
    ...
    WEBFINDMETHOD = (('find_element_by_id', 'id'),('find_element_by_name', 'name'),('find_element_by_class_name', 'class_name'),('find_element_by_xpath', 'xpath'))
    webfindmethod = models.CharField(verbose_name='定位方式',choices = WEBFINDMETHOD,max_length=200)# 定位方式
    #webfindmethod = models.CharField('定位方式',max_length=200)      # 定位方式

    webevelement = models.CharField('控件元素',max_length=800)       # 控件元素

    WEBOPTMETHOD = (('click', 'click'),('sendkeys', 'sendkeys'),('check', 'check'))
    weboptmethod = models.CharField(verbose_name='操作方法',choices = WEBOPTMETHOD,max_length=200) # 操作方法
